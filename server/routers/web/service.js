...
//handle部分
// 先收藏,第二次收藏时判定有没有收藏过,如果有就收藏不了,如果没有就可以收藏
exports.collection = async (req, res) => {
	// const sql1 =  `select * from collection where article_title like '%${req.body.article_title}%'`
	const sqlStr = 'select * from collection where username = ? and article_title = ?'
	let info = [req.body.username,req.body.article_title]
	const data1 = await query(sqlStr, info)
	// res.send(data1.length)
	if(!data1.length){
		const sqlc =  'insert into collection set ?'
		const data2 = await query(sqlc,{username:req.body.username,article_title:req.body.article_title,author:req.body.author})
		res.send("收藏成功")
	}else{
		res.send("收藏失败")
	}
}
// 收藏成功后，前端通过用户名去获取收藏的文章名,通过发送的名字返回相同名字拥有的文章名字
exports.getcollectionlist = async (req,res) =>{
	const sqlStr = 'select * from collection where username = ?'
	let info = req.body.username
	const data = await query(sqlStr,info)
	const list = []
	// const obj = {
	// 	article_title:''
	// }
	data.forEach((element)=>{
		// obj.article_title = element
		list.push(element)
	})
	res.send(list)
}
...
exports.cancelcollection = async (req,res) =>{
	const sql = 'delete from collection where username = ? and article_title = ? '
	// let info = [req.body.username,req.body.article_title]
	const data = await query(sql,[req.body.username,req.body.article_title])
